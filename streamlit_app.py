...
if completely_new_session or not st.session_state.chat_history:
    start_new_session()
...
with st.form(key='my_form', clear_on_submit=True):
    ...
    if user_input:
        ...
        if st.session_state.session_id:
            st.session_state.chat_history[st.session_state.session_id].append((user_input, "AI's response here."))
    submit_button = st.form_submit_button(label='Send')

if st.session_state.session_id:
    st.header(f"Session {st.session_state.session_id}")
    session_history = st.session_state.chat_history.get(st.session_state.session_id, [])
    for question, answer in session_history:
...
template = (
    ...
    the best answer should be 'Yes we have,
    ...
    day, date or time kindly engage by asking for these specifics. {details} Use these details that is today's date and day /
    ...
    If the appointment schedule is not available provide this link: www.dummy_calenderlink.com to schedule the appointment by the user himself. /
...
details = "Today's current date is " + current_date + " and today's week day is " + current_day + "."
...
def conversational_chat(user_input):
    ...
    st.session_state.chat_history[st.session_state.session_id].append((user_input, result["output"]))
...
with container:
    ...
    if st.session_state.user_name and st.session_state.chat_history[st.session_state.session_id]:
        current_session_data = {
            'user_name': st.session_state.user_name,
            'chat_history': st.session_state.chat_history[st.session_state.session_id]
        }
        st.session_state.past.append(current_session_data)

with response_container:
    for i, (query, answer) in enumerate(st.session_state.chat_history[st.session_state.session_id]):
